...
class DataProcess:
    ...
    def load_data(self, paths, mode='train'):
        ...
        print('begin load data from ' + str(path))
        ...
        data = np.empty((tot_num_frames, self.image_height, self.image_width ,  1),
        ...
        for i, frame in enumerate(self.generate_frames(path, mode_person_ids)):
            ...
            data[i,:,:,0] = (cv2.resize(
                frame_np, (self.image_height, self.image_width))/255.0).astype(np.float32)
