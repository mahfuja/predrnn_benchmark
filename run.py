...
def train_wrapper(model):
    ...
    name = 'Summary_' + str(args.model_name) + '.csv'
    file_name = os.path.join(args.gen_frm_dir, name)
    ...
    for itr in range(1, args.max_iterations + 1):
        ...
        if itr % args.test_interval == 0:
            summary_dict["itr"] = itr
            summary_dict["train_loss"] = train_loss
            avg_mse, ssim, psnr, lp = trainer.test(model, test_input_handle, args, itr)
            summary_dict["avg_mse"] = avg_mse
            summary_dict["ssim"] = ssim
            summary_dict["psnr"] = psnr
            summary_dict["lpips"] = lp
...
def test_wrapper(model):
    ...
    avg_mse, ssim, psnr, lp = trainer.test(model, test_input_handle, args, 'test_result')
    name = 'Summary_' + str(args.model_name) + '.csv'
    file_name = os.path.join(args.gen_frm_dir, name)
    summary_dict["pretrained_model"] = args.pretrained_model
    summary_dict["avg_mse"] = avg_mse
    summary_dict["ssim"] = ssim
    summary_dict["psnr"] = psnr
    summary_dict["lpips"] = lp
